    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
            <div className="w-full max-w-md rounded-2xl border border-white/10 bg-ink-800 p-6" onClick={(e) => e.stopPropagation()}>
                <h3 className="text-lg font-black text-white">{t('Request a call')}</h3>
                <p className="mt-1 text-sm text-gold">📅 {slot.dayLabel} · {slot.label} (1 hour)</p>
                <form onSubmit={submit} className="mt-5 space-y-4">
                    <div>
                        <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-400">{t("What's it about?")}</label>
                    ...
                    <div>
                        <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-400">{t('Anything else? (optional)')}</label>
...
export default function Booking({ days, bookings }) {
    ...
    return (
        <PanelLayout title="Book a Call">
            ...
            <div className="mb-5">
                <h2 className="text-2xl font-black text-white">{t('Book a call with Taha')}</h2>
            ...
            {/* Your calls */}
            <div className="mt-8">
                <h2 className="mb-4 text-lg font-black text-white">{t('Your calls')}</h2>
                {bookings.length === 0 ? (
                    <p className="rounded-2xl border border-white/5 bg-ink-700 p-6 text-sm text-gray-400">{t("You haven't booked any calls yet.")}</p>
                ) : (
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        {bookings.map((b) => (
                            <div key={b.id} id={`booking-${b.id}`} className="rounded-2xl border border-white/5 bg-ink-700 p-4">
                                <div className="flex items-center justify-between gap-2">
                                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${STATUS[b.status]?.badge}`}>{STATUS[b.status] ? t(STATUS[b.status].label) : b.status}</span>
                                    {!b.is_past && (b.status === 'pending' || b.status === 'confirmed') && (
                                        <button onClick={() => cancel(b)} className="text-xs text-red-400 hover:text-red-300">{t('Cancel')}</button>
                                    )}
                                </div>
