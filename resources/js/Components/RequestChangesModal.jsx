        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
            <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-ink-800 p-6" onClick={(e) => e.stopPropagation()}>
                <div className="mb-4 flex items-center justify-between">
                    <h3 className="text-lg font-bold text-white">{t('Request changes')}</h3>
                    <button onClick={onClose} className="text-gray-400 hover:text-white">✕</button>
                </div>
                <p className="mb-3 text-sm text-gray-400">
                    {t('Tell Taha what to adjust on')} <span className="text-gold">“{task.title}”</span>. {t('He\'ll get it back to “in progress”.')}
                </p>
                <textarea
                    autoFocus
                    rows={5}
                    ...
                    placeholder={t("Describe the changes you'd like… (e.g. the header color, missing contact form, a bug on mobile)")}
                ...
                <div className="mt-4 flex justify-end gap-3">
                    <button onClick={onClose} className="rounded-full border border-white/15 px-5 py-2 text-sm text-gray-300 hover:text-white">{t('Cancel')}</button>
